...
class CISTransformerEncoder(nn.Module):
    ...
    def forward(self, srcs, pos_embeds):
        ...
        for lvl, (src, pos_embed) in enumerate(zip(srcs, pos_embeds)):
            bs, c, h, w = src.shape
            # c, h, w = src.shape                 d_model=256, d_ffn=1024,
        ...
        # encoder
        memory = self.encoder(src_flatten, spatial_shapes, level_start_index, spatial_shape, lvl_pos_embed_flatten)
...
class TransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        # self.depthwise = nn.Conv2d(256, 256, 3, 1, 1, groups=256, bias=False)

    def forward(self, src, pos, reference_points, spatial_shapes, level_start_index, spatial_shape, padding_mask=None):
        # self attention
        # pos = None
...
class TransformerEncoder(nn.Module):
    ...
    def forward(self, src, spatial_shapes, level_start_index, spatial_shape, pos=None):
        output = src
        batch_size = src.shape[0]
        reference_points = get_reference_points(spatial_shapes, batch_size, device=src.device)
        for _, layer in enumerate(self.layers):
            output = layer(output, pos, reference_points, spatial_shapes, level_start_index, spatial_shape)
...
def build_transformer_encoder(cfg):
    return CISTransformerEncoder(
        d_model=cfg.MODEL.GLNet.HIDDEN_DIM,
        nhead=cfg.MODEL.GLNet.NHEAD,
        num_encoder_layers=cfg.MODEL.GLNet.ENC_LAYERS,
        dim_feedforward=cfg.MODEL.GLNet.DIM_FEEDFORWARD,
        dropout=0.1,
        ffn_type=cfg.MODEL.GLNet.FFN,
        # num_feature_levels=len(cfg.MODEL.GLNet.FEAT_INSTANCE_STRIDES),
        num_feature_levels=4,
        enc_n_points=cfg.MODEL.GLNet.ENC_POINTS)
