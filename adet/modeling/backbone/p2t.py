...
class PyramidPoolingTransformer(Backbone):
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 320, 512],
                 ...
                 depths=[2, 2, 9, 3], out_features=None): #
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        output_features[self._out_features[0]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        output_features[self._out_features[1]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        output_features[self._out_features[2]] = x
        ...
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2)
        output_features[self._out_features[3]] = x
...
def p2t_tiny(cfg, input_shape):

    model = PyramidPoolingTransformer(out_features=["res2", "res3", "res4", "res5"],
...
def p2t_small(cfg, input_shape):
    ...
    out_features = ["res2", "res3", "res4", "res5"]
    out_feature_channels = {"res2": 64, "res3": 128,
                            "res4": 320, "res5": 512}
    out_feature_strides = {"res2": 4, "res3": 8, "res4": 16, "res5": 32}
    model._out_features = out_features
    model._out_feature_channels = out_feature_channels
