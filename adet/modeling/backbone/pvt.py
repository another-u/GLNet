...
class PyramidVisionTransformer(Backbone):
    def __init__(self, cfg, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 6, 3], sr_ratios=[8, 4, 2, 1], num_stages=4, out_features=None):
        super().__init__()
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stages):
            ...
            output_features[self._out_features[i]] = x
...
def pvt_tiny(cfg, input_shape):
    model = PyramidVisionTransformer(
        cfg, out_features=cfg.MODEL.PVTV2.OUT_FEATURES,
        patch_size=4, embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2, 2, 2], sr_ratios=[8, 4, 2, 1])
    # model.default_cfg = _cfg()
    out_feature_channels = {"res2": 64, "res3": 128, "res4": 320, "res5": 512}
