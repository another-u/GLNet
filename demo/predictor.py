...
class VisualizationDemo(object):
    ...
    def run_on_image_sod(self, image):
        predictions = self.predictor(image)
        try:
            sod = predictions['instances'].seg_preds.sum(0).cpu().numpy()
            sod[sod > 1] = 1
            sod = sod * 255
        except Exception:
            sod = np.zeros_like(image)
        return sod
    ...
    def run_on_image(self, image):
        ...
        Returns:
            predictions (dict): the MSPNet_update_v1_p2t_tiny_RT_e6_d3_v2_trans_e_d_updated of the model.
            vis_output (VisImage): the visualized image MSPNet_update_v1_p2t_tiny_RT_e6_d3_v2_trans_e_d_updated.
        """
        vis_output = None
        predictions = self.predictor(image)
        ...
        if "panoptic_seg" in predictions:
            ...
            if "pred_sems" in predictions:
                sem_pred = predictions["pred_sems"].squeeze(0).to(self.cpu_device)
                # plt.imshow(sem_pred * 255, cmap='gray')
                # plt.show()
